...
stackName=$4
version=$5
cbVersion = ${version:0:3}
osName = "centos6"
if [ $(echo "$cbVersion >= '6.5'" | bc) -eq 1 ]
then
    osName = "amzn2"
fi
...
wget https://packages.couchbase.com/releases/${version}/couchbase-server-enterprise-${version}-${osName}.x86_64.rpm
rpm --install couchbase-server-enterprise-${version}-${osName}.x86_64.rpm
